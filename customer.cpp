...
int Customer::CustomerID() const { return customerID; }

string Customer::FirstName() const { return first_name; }

string Customer::LastName() const { return last_name; }
// attached the borrwing moies type on the customer borrwed movie list
void Customer::Borrow(char genre, int index) { borrowed[index] = genre; }
// accept Transaction object as a parameter and add into customer transcation
// story.
...
void Customer::outputHistory() {
  ...
  cout << "History transactions for " << FirstName() << " " << LastName()
       << " #" << CustomerID() << ":" << endl;
