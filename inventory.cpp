...
void Inventory::addClassic(int stock, const string &director,
  ...
  int index = indexOfClassic(releaseMonth, releaseYear, majorActorFirstName,
                             majorActorLastName);
  if (index > -1) {
    // update movie stock
...
bool Inventory::borrowMovie(const string &type, int index) {
  ...
  if (type == "D") {
    ...
    // check to make sure there is avialbale stock to rent
    if (stock - 1 < 0) {
      cout << "out of stock " << dramas[index]->Title() << endl;
      return false;
    }
    // decrease the stock and increase the number of borrow movie
  ...
  if (type == "C") {
    ...
    // check to make sure there is avialbale stock to rent
    if (stock - 1 < 0) {
      cout << "out of stock " << classics[index]->Title() << endl;
      return false;
    }
    // decrease the stock and increase the number of borrow movie
...
bool Inventory::sortDrama(Drama *drama1, Drama *drama2) {
  return (drama1 < drama2);
}
